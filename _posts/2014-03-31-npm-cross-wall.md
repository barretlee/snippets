...
在配置中直接指定源头，下次就没有必要使用 --registry 参数了。
...
这样配置好了之后，打开你的 goagent ，记得一定要打开，不然 npm 必然报错。

### 方法三

直接下载到本地，实在是怕麻烦，就直接把文件 download 下来，然后放到 node_module 之中就行了。如果是全局模块，找到全局 node_module 的位置，然后解压放进去就行了。
